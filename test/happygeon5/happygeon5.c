// https://syzkaller.appspot.com/bug?id=7d455fd782d2d1297d586aa3353bcaa0d758665d
...
void execute_one(void)
{
  ...
  if (res != -1)
    r[0] = res;
  NONFAILING(syz_usb_connect(2, 0, 0, 0));
  ...
  NONFAILING(*(uint64_t*)0x20000014 = 0x20000040);
  NONFAILING(*(uint8_t*)0x20000040 = 0);
  NONFAILING(*(uint8_t*)0x20000041 = 0x22);
  NONFAILING(*(uint32_t*)0x20000042 = 5);
  NONFAILING(STORE_BY_BITMASK(uint8_t, , 0x20000046, 3, 0, 2));
  NONFAILING(STORE_BY_BITMASK(uint8_t, , 0x20000046, 2, 2, 2));
  NONFAILING(STORE_BY_BITMASK(uint8_t, , 0x20000046, 0, 4, 4));
  NONFAILING(memcpy((void*)0x20000047, "\xf9\xf4\x17\x8e", 4));
  NONFAILING(*(uint64_t*)0x2000001c = 0);
  NONFAILING(syz_usb_control_io(r[0], 0x20000000, 0));
  NONFAILING(*(uint32_t*)0x20001640 = 2);
  NONFAILING(*(uint32_t*)0x20001644 = 2);
  NONFAILING(*(uint32_t*)0x20001648 = 6);
  NONFAILING(*(uint32_t*)0x2000164c = 0x7fff);
  NONFAILING(*(uint32_t*)0x20001650 = 0x1000);
  NONFAILING(*(uint32_t*)0x20001654 = 1);
  syscall(__NR_ioctl, -1, 0x4018480c, 0x20001640ul);
  NONFAILING(memcpy((void*)0x200001c0,
                    "\x9a\x04\x00\x00\x00\xb0\xc0\xea\xc1\x00\x90\x23\xff\x1f"
                    "\x9d\x09\x00\x00\x00\x00\x80\x00\x00\x79\xce\x9a\x13\x5f"
                    "\x56\x38\x45\xce\xe9\x32\x4a\xb3\x54\xa5\x11\xcc\x86\x6c"
                    "\x2a\x01\x00\xf0\x6f\xea\x7f\x1d\x07\xdf\x6d\xae\xda\x54"
                    "\xd0\xde\x1b\xde\xd6\x0c\xbd\x14\x32\x51\x69\x49\xe0\x75"
                    "\x16\x5c\xc0\x32\x81\x9f\xfb\x66\x04\xf7\x50\x00\x00\x00"
                    "\x00\x00\x00\x00\x00\x00\x47",
                    91));
  NONFAILING(syz_usb_ep_write(r[0], 0, 0x5b, 0x200001c0));
}
int main(void)
{
